...
class ChuLi51Spider(Spider):
    ...
    async def get_list(self,response):
        if response.meta["page"] > 1 and response.meta["page"] <= 200:
    ...
    async def get_page(self,response):
        ...
        doc_["machinePublishTime"] = datetime.strptime(pub_time[0],"%Y-%m-%d %H:%M").strftime("%Y-%m-%d %H:%M:%S") if pub_time else ""
